...
impl LeptonHeader {
    ...
    pub fn read_compressed_lepton_header<R: Read>(
        ...
        self.rinfo.raw_jpeg_header = self
            .read_lepton_compressed_header(&mut compressed_reader, enabled_features)
            .context()?;
...

        {
            ...
            // SAGETHUMBS PATCH (0.5.8): `parse` returns Ok(false) when the raw JPEG
            // header ends at EOI before any SOS (no scans). The boolean was ignored;
            // a hostile ~80-byte .lep (JPEG header `FF D9` + an HH marker) then
            // reached `truncate_components.init` with cmpc==0 and the subsequent
            // `get_block_height(0)` indexed an empty trunc_info — a Vec
            // out-of-bounds PANIC in ALL builds (release included, panic=abort)
            // inside dllhost/explorer. Requiring a real scan header also blocks the
            // sibling huffman-index panic (huff_dc/huff_ac stay 0xff without an SOS,
            // indexing h_codes[0][255] during recode).
            if !self
                .jpeg_header
                ...
                .context()?
            {
                return err_exit_code(
                    ExitCode::BadLeptonFile,
                    "JPEG header ended before any scan (SOS)",
                );
            }
        ...
        // SAGETHUMBS PATCH (0.5.8): legacy C++-encoded files (e.g. dropbox/lepton
        // gold-legacy.lep) can carry ZERO thread handoffs; `num_threads - 1` below
        // then underflows and panics. Reject such a header with a clean error
        // instead of aborting (the caller falls through to the next decode tier).
        if num_threads == 0 {
            return err_exit_code(
                ExitCode::BadLeptonFile,
                "no thread handoffs in lepton header",
            );
        }
        ...
        // SAGETHUMBS PATCH (0.5.8): validate the thread ranges now. Hostile
        // luma-split (HH) values — inverted, overlapping, or non-contiguous
        // ranges — later trip BlockBasedImage::merge's contiguity assert
        // (`dpos_offset == contents.len()`), a PANIC in ALL builds on the main
        // thread for progressive files. Legit encoders always write contiguous
        // ranges starting at row 0 (the format requires it for merge); reject
        // anything else as a clean BadLeptonFile.
        for i in 0..num_threads {
            if self.thread_handoff[i].luma_y_start > self.thread_handoff[i].luma_y_end {
                return err_exit_code(ExitCode::BadLeptonFile, "inverted thread luma range");
            }
            if i > 0
                && self.thread_handoff[i].luma_y_start != self.thread_handoff[i - 1].luma_y_end
            {
                return err_exit_code(ExitCode::BadLeptonFile, "non-contiguous thread luma ranges");
            }
        }
        if self.thread_handoff[0].luma_y_start != 0 {
            return err_exit_code(
                ExitCode::BadLeptonFile,
                "first thread luma range must start at row 0",
            );
        }
        ...
        if self.rinfo.early_eof_encountered {
            // SAGETHUMBS PATCH (0.5.8): the plain `-` arithmetic here underflows and
            // PANICS (debug builds / overflow checks) on files encoded by the legacy
            // C++ lepton whose jpeg_file_size is smaller than garbage + header + SOI
            // (e.g. dropbox/lepton test image gold-legacy.lep). Saturating math keeps
            // the "shorten the last segment" intent: a size that doesn't fit simply
            // clamps to 0, the segment decode fails cleanly, and the caller gets an
            // error instead of an abort inside the shell host.
            let mut max_last_segment_size = self
                .jpeg_file_size
                .saturating_sub(u32::try_from(self.rinfo.garbage_data.len())?)
                .saturating_sub(u32::try_from(self.raw_jpeg_header_read_index)?)
                .saturating_sub(u32::try_from(SOI.len())?);

            // subtract the segment sizes of all the previous segments (except for the last)
            for i in 0..num_threads - 1 {
                max_last_segment_size =
                    max_last_segment_size.saturating_sub(self.thread_handoff[i].segment_size);
            }
    ...
    fn read_lepton_compressed_header<R: Read>(
        &mut self,
        src: &mut R,
        enabled_features: &EnabledFeatures,
    ) -> Result<Vec<u8>> {
        // SAGETHUMBS PATCH (0.5.8): every length field in this function comes
        // straight from the (zlib-compressed, hostile-controllable) header; the
        // upstream code resized vectors with no bound, so a ~40-byte file
        // declaring ~4 GiB lengths OOM-aborted the process (panic=abort, in the
        // shell host). Cap all of them at the declared-JPEG ceiling; anything
        // larger is not a plausible header and is rejected as BadLeptonFile.
        let cap = |n: usize| -> Result<usize> {
            if n > enabled_features.max_jpeg_file_size as usize {
                return err_exit_code(
                    ExitCode::BadLeptonFile,
                    "header length exceeds max_jpeg_file_size",
                );
            }
            Ok(n)
        };
        ...
        let hdrs = cap(header_reader.read_u32::<LittleEndian>()? as usize)?;
        ...
        loop {
            ...
            if buffer_prefix_matches_marker(current_lepton_marker, LEPTON_HEADER_PAD_MARKER) {
                ...
                let rst_err_count = cap(header_reader.read_u32::<LittleEndian>()? as usize)?;
                ...
                let garbage_size = cap(header_reader.read_u32::<LittleEndian>()? as usize)?;
