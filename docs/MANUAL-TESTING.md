...
1. 右键 `photo.jpg` → **Convert into ▸ LEP** 必须出现在菜单中。
2. 点击 → 预期：同目录生成 `photo.lep`，无错误弹窗。
3. 验证无损：
...
1. 右键一张 PNG（或 GIF/WebP 等非 JPEG）→ **Convert into ▸ 子菜单中必须没有 LEP 项**（不再显示后报错）。
2. 多选混合（1 张 JPEG + 1 张 PNG）→ 右键 → Convert into ▸ 中同样**没有 LEP 项**（任一非 JPEG 即隐藏）。
3. 全选 JPEG（可含 .jpeg/.jpe/.jfif/.JPG 大写）→ LEP 项出现。
4. CLI 拒绝路径仍然有效（菜单隐藏只影响右键入口，CLI/对话框仍按原契约报错）：
...
- **Win11 现代右键菜单的 LEP 子项始终显示**（平台限制：现代菜单子命令枚举无文件选择上下文，无法按选择过滤；非 JPEG 点击会报错）。本 Compact 包未注册现代菜单，不受影响；经典菜单已按选择过滤。
