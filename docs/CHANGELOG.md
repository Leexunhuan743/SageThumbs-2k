...
- **Lepton is now an export format too.** "Convert into ▸ LEP" and the Convert… dialog's **LEP** target recompress JPEG-family sources (JPG/JPEG/JPE/JFIF) **losslessly** — the container is a bit-exact copy of the JPEG bytes, so EXIF/ICC markers survive untouched and a typical photo shrinks ~20%. Resize/rotate of a `.lep` keeps `.lep` (decoded, edited, recompressed at your JPEG quality, like any `.jpg` edit). Non-JPEG sources get an explicit "needs a JPEG source" error (the dialog disables the LEP entry with a note when the selection isn't all JPEG). `st2k convert in.jpg out.lep`, `st2k batch --to lep` and the MCP convert tool all route through the same lossless path.
...
- **Encoding can no longer abort the shell either.** The encode side was never audited — it owns a JPEG entropy decoder the thumbnail path never touches — so it now carries three more vendored patches: an output-failure path that used to panic (and under panic=abort kill dllhost/explorer) on a full disk, and two debug-only integer overflows reachable from hostile Huffman tables and progressive scans. Encode is fuzzed against 400 deterministic JPEG mutations, and concurrent encodes are capped at 2 (~2 GiB worst case) by a named gate that mirrors the ImageMagick one.
